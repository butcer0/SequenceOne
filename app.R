...
# doSomething <- function(message) {
#    print(paste("doing something:", message));
#    printLog(paste("doing something:", message));
# }
# 
# getContext <- function(request) {
#    return(content(request, as = "text", encoding = "UTF-8"))
...
# }
# 
# getSequence <- function(sequenceLocs, motifLength, geneId, tF, offset) {
#    request <- GET(url = GET_SEQUENCE_ENDPOINT,
#                   query = list(
#                      database = "epdNew_hg",
#                      gene_id = geneId,
...
# }
# 
# # getSequences <- function(consensus_regions_list, offset) {
# #    # print(paste("Gene:",GENE_IDS[1],"-","Trans Factor:", TF_NAME))
# #    for (consensus_regions in consensus_regions_list) {
# #       for (location in consensus_region) {
# #          sequence <- getSequence(location, MOTIF_LENGTH, offset)
# #          print(paste("    location:", location, "-", sequence));
# #          printLog(paste("    location:", location, "-", sequence))
# #       }
# #    }
# # }
# # 
# # getLocationsAndSequences <- function(gene_ids, motifs, from, to, p, offset) {
# #    print("getting genes and locations")
# #    # printLog("getting genes and locations")
# #    
# #    result_sequences <- data.frame(NULL)
# #    for (gene_id in gene_ids ) {
# #       for (row in 1:nrow(motifs)) {
# #          print(paste("gene:",gene_id, "motif_id", motifs[["motifIds"]][row], "motif_names", motifs[["motifNames"]][row]))
# #          printLog(paste("gene:",gene_id, "motif_id", motifs[["motifIds"]][row], "motif_names", motifs[["motifNames"]][row]))
# #          request <- GET(url = PLOT_GENE_ENDPOINT, 
# #                         query = list(
# #                            database = "epdNew_hg",
# #                            gene_id = gene_id,
...
# #          
# #          print(paste("Request:", request))
# #          # printLog(paste("Request:", request))
# #          html_output <- read_html(request, encoding="UTF-8")
# #          p_consensus_regions <- parseSequenceLocations(html_output)
# #          # print(p_consensus_regions)
# #          # browser()
...
# #          sequences <- c()
# #          for (location in p_consensus_regions) {
# #             sequence <- getSequence(location, MOTIF_LENGTH, gene_id, tf, offset)
# #             seq_set <- data.frame(as.list(location, sequence))
# #             sequences <- append(sequences, sequence)
# #             print(paste("    location:", location, "-", sequence));
# #          }
# #          
# #          print(paste("consensus regions:",p_consensus_regions))
# #          # printLog(p_consensus_regions)
# #          print(paste("sequences:",sequences))
# #          # printLog(sequences)
# #          
# #          current_sequences <- data.frame(gene= rep(gene_id, times = length(sequences)), motif = rep(motifs[["motifNames"]][row], times = length(sequences)), site = I(p_consensus_regions), sequence = I(sequences))
# #          result_sequences <- rbind(result_sequences, current_sequences)
...
#    print(paste("logging", log))
#    Log_Output <- paste(Log_Output, log, sep = "\n")
# }
# 
...
#          div(style="display: inline-block;vertical-align:top; width: 150px;", sliderInput("offset_input", "Offset",
#                                                                                            min = -12, max = 0,
#                                                                                            value = OFFSET)),
#          textOutput("result")
#       ),
...
#             helpText('Please continue updating values while the data is loading. The data view will continually update to reflect any changes.')
#          ),
#          # verbatimTextOutput("print_log")
#          # div(style="display: inline-block;vertical-align:top; width: 100%;overflow-y: auto; max-height: 200px",  textOutput("print_log")),
#       )
#    )
# )
# 
...
#       # names(sequenceDT) <- c("motifIds", "motifNames")
#       if (length(motifs[[1]]) > 0 && length(input$gene_names_input) > 0) {
#         print(paste("offset:", input$offset_input))
#          sequenceDataTable <- getLocationsAndSequences(input$gene_names_input, motifs, input$from_input, input$to_input, input$p_input, input$offset_input)
#          view(sequenceDataTable)
#          return(sequenceDataTable)
#       } else {
#          # return(invisible(NA))
#         return(data.frame(N))
